...
private _nearbyId = FLO_ObjectiveAreaNearbyId;
private _recordIds = keys FLO_ObjectiveClientObjectiveRecords;
_recordIds sort true;

private _areas = [];
private _areaIds = createHashMap;
private _firstId = "";
private _nearbyHeldId = "";

{
    private _record = FLO_ObjectiveClientObjectiveRecords get _x;
    ...
    if ((_ownerKey isEqualTo _playerSideKey) && {_objectiveState isEqualTo "held"}) then {
        ...
        _areaIds set [_objectiveId, true];

        if (_firstId isEqualTo "") then {
            _firstId = _objectiveId;
        };

        if (_isNearby) then {
            _nearbyHeldId = _objectiveId;
        };
    };
} forEach _recordIds;

private _selectedId = FLO_ObjectiveAreaActiveId;

if !(_selectedId in (keys _areaIds)) then {
    _selectedId = [_firstId, _nearbyHeldId] select (_nearbyHeldId isNotEqualTo "");
    FLO_ObjectiveAreaActiveId = _selectedId;
};
